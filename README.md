...
scripts/check.py               <- daily: `health` probes all 3 LLMs and every feed -> dashboard
                                  badges; `agreement` measures inter-rater reliability (kappa)
...
- (Optional) the workflow sets GEMINI_MODEL to gemini-2.5-flash, GROQ_MODEL to
  ...
  free equivalent — `scripts/check.py health` checks all three daily and the
...
and when it was last checked. Each daily run, `check.py health` pings all
