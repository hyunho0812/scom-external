...
cards           수집 이벤트 / 트래픽 변화 / 방향 적중률(%) / 누적 영향지수 상관
split           트래픽 추세 차트  |  3축 진단(요약 3줄 + 축 행 + 변화 기여 비중)
axisDetail      3축 전체 폭 상세
외부 요인       [정렬·방향·축 세그먼트]  →  split( 목록 | 선택한 요인 상세 )
```

- **목록 필터는 목록에만 건다.** `FDIR`/`FAXIS`/`FSORT`는 `lr`(목록용)에만 적용하고
  3축 분해는 언제나 기간 전체 `r`을 쓴다 — 분해는 항등식이라 부분집합으로 계산하면
  퍼센트가 서로 안 맞는다.
- **변화 기여 비중** = `|eff_axis| / Σ|eff|`. 부호를 뺀 **크기** 비중이고, 방향은
