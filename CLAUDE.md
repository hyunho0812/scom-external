...
keep율은 낮아질 수 있음 — `data/feed_performance.json`으로 소스별 kw_pass_rate/keep_rate
추적 가능해짐, 2026-07-14 추가. `optimize.py`가 이걸 `kw_feeds.txt` 튜닝 프롬프트에
넣어줌 — keep_rate가 낮은 소스는 "주제는 맞는데 전망 위주라 계속 걸러짐"으로 해석).
