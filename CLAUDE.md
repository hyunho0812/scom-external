...
- `axis`: 대시보드 3축 진단 패널(수요/점유/공급)이 이 이벤트를 어느 축에 배정할지 —
  ...
  **2026-08-28: 그 재판정을 `maintenance.py axis --rejudge`로 만들었다** (API 키 필요).
  ...
  - **옛 휴리스틱 값은 저장하지 않는다.** 한때 `axis_heuristic`에 남겼다가 2026-08-28
    ...
    description)이 이벤트에 그대로 있다. 실측으로 94건 전부 재계산해
  ...
  **2026-08-28: 94건을 실제로 다시 판정했다 (`axis_source: "claude-opus-5"`).**
  ...
  입력은 `--rejudge`가 모델에 넘기는 것과 같은 `title/description/impact`다.
...
관측된 변화 대비 몫이라 그렇다. **목록의 행**은 그래서 이벤트의 방향이 아니라 **몫의
부호**로 칠한다 — 음수를 초록으로 칠하면 숫자와 반대되는 말을 하게 된다.

**요약 줄에는 그 규칙을 쓰면 안 된다 (2026-09-02).** 행에는 라벨이 없고 숫자만 있지만,
요약에는 그룹 이름이 붙기 때문이다. 옛 요약은 `증가 요인 합 −102.1%`를 **하락과 같은
빨강**으로 칠했고, 라벨은 "증가"라는데 숫자와 색은 하락을 말해 서로 싸웠다. 실제 뜻은
"증가 요인이 이 하락의 102.1%를 되돌렸다"이다. 고친 방식 셋:
- **행 이름을 방향이 아니라 이 기간에서의 역할로 붙인다.** `하락을 만든 요인 +202.1%` /
  `하락을 되돌린 요인 −102.1%`. **몫이 양수면 관측된 움직임을 민 쪽, 음수면 되민 쪽**이라
  방향과 무관하게 성립하는 규칙이고, 이러면 라벨과 부호가 같은 말을 한다. 원래 방향은
  `▼감소 288건`처럼 부제로 남긴다.
- **숫자에서 색을 뺐다.** 값은 전부 `--ink`이고, 색은 앞의 점과 막대만 갖는다. 값에
  계열색을 입히는 것이 애초에 라벨과 색이 싸우게 된 원인이다.
- **색은 부호가 아니라 역할을 따른다.** 만든 쪽이 그 기간 움직임의 색(하락이면 `--neg`),
  되민 쪽이 반대색. 요인 구성과 트래픽이 반대인 conflict 기간에는 역할을 말할 수 없으므로
  `몫이 +인 요인`으로 물러서고 점은 `--neu` 회색이 된다.
- 이벤트가 0건인 그룹은 행을 만들지 않는다 — `+0.0%`로 찍으면 "하락을 만든 요인"이 한
  화면에 두 번 나온다(실제로 2026-01·02·11·12월이 그랬다).

**상승 분기는 저장된 데이터로는 검증할 수 없다.** 위키 시계열이 전 구간 하락이라
**선택 가능한 54개 기간 중 트래픽이 오른 기간이 0개**다. `하락/상승` 낱말을 `totalPct`
부호에서 뽑아 쓰도록 짜 놓아도, 화면에서는 늘 `하락`만 나온다는 뜻이라 그대로 두면
테스트되지 않은 코드가 된다. **실측 CSV 업로드 경로로 강제해서 확인했다**(브라우저
메모리에서만 도는 그 경로가 총량을 바꾸므로 같은 코드가 그대로 실행된다):
- 상승 + 증가 우세 → `상승을 만든 요인 ▲증가 58건 +231.3%` / `상승을 되돌린 요인
  ▼감소 40건 −131.3%`. 색도 역할을 따라 뒤집힌다(만든 쪽 `--pos`).
- 상승인데 원장은 감소 우세 → conflict로 정확히 떨어져 `몫이 +인 요인`으로 물러선다.
**다음에 이 패널을 고치면 같은 방법으로 두 분기를 다시 확인할 것.**

**색은 이제 세 번째 채널일 뿐이다.** `--pos`/`--neg`는 적록 쌍이라 dataviz 검증기로 재보니
**deutan ΔE 2.8**(정상시야 28.2)로, 적록색약에게는 사실상 구분되지 않는다. 대시보드 전체가
쓰는 토큰이라 이번에 바꾸지 않았고, 대신 이 패널에서는 **글자 라벨 + ▲/▼ 기호 + 0선의
어느 쪽인지(위치)** 로 같은 정보를 세 번 말한다. 색만으로 읽어야 하는 곳이 없다.
...
`window_mean()`을 쓰면서 커버리지를 안 보는 코드는 세 번 다 버그였다.

**2026-08-31(같은 날, 세 번째): `avgInRange()`(build.py JS)에도 있었다.** 기본 기간이
양쪽 100% 커버라 무해해 보였지만, 기간 드롭다운이 만들 수 있는 **54개 조합 중 6개**가
시계열이 없는 창을 평균 내고 있었다(시계열 2024-08-30~2026-08-29):

| 고른 기간 | 짧은 쪽 | 충족률 | 화면에 뜨던 값 |
|---|---|---|---|
| 월 2025-08 | 비교 2024-08 | **0.06** | −5.0% |
| 연 2025 | 비교 2024 | 0.34 | −21.3% |
| 분기 2025Q3 | 비교 2024Q3 | 0.35 | −25.0% |
| 분기 2026Q3 | 현재 | 0.65 | −34.6% |
| 연 2026 | 현재 | 0.66 | −40.6% |
| 월 2026-08 | 현재 | 0.94 | −37.9% |

가장 나쁜 건 **월 2025-08**이다 — 한 달 평균을 2024-08의 **2일**과 비교하고 −5.0%라고
적고 있었다. 앞쪽 꼬리(백필 시작점)라 창을 잘라 맞추는 것으로도 못 고친다.

고친 방식은 파이썬과 같다. **검사를 호출부가 아니라 `avgInRange()` 안에 넣었다** —
호출부에 두는 설계였기 때문에 파이썬에서 두 번 빠뜨렸다. 딸린 조각 셋:
- `MIN_COVER` 0.95 = `score_predictions.MIN_WINDOW_COVERAGE`. 같은 기준을 두 벌로
  적지 않으려면 언젠가 한쪽에서 읽어야 하지만, JS는 상수 주입이 따로 필요해 지금은
  주석으로만 묶어 두었다.
- **충족률은 날짜 수가 아니라 시계열 자신의 주기로 잰다**(`seriesStep()`, 관측된 간격의
  중앙값). 위키는 일별이고 **CrUX는 주별**이라, 달력 날짜로 세면 CrUX 창이 전부 1/7로
  떨어져 공급축 보정이 조용히 사라진다.
- `trendVerdict()`의 `avg`와 `compVerdict()`의 `avgSum`이 **`avgInRange()`의 사본**이었다
  (원칙 19와 같은 모양). 파이썬 쪽을 고쳤을 때 여기가 안 고쳐진 이유가 그것이다.
  공용 함수로 돌려 세 벌을 한 벌로 만들었다.

**모자란 창은 숫자를 깎는 대신 계산을 안 한다.** 그리고 화면이 왜 비었는지 말한다 —
"선택한 비교 기간이 트래픽 데이터 범위를 벗어납니다 — 6%만 있어 … (보유 구간
2024-08-30 ~ 2026-08-29)". 아직 안 끝난 기간이면 `누적` 옵션을 쓰라고 덧붙인다
(`이번 달 누적`은 같은 8월을 100% 충족으로 비교한다). 조용히 비면 고장으로 읽힌다.

검증: 헤드리스 크롬으로 54개 기간을 이전 빌드와 나란히 돌렸다. **숫자가 있던 well-covered
기간 17개는 3축 행·배분 요약까지 바이트 단위로 동일**하고, 위 6개만 값이 사라졌다.
나머지 31개는 이전에도 비어 있었고 문구만 "기간 비교 필요"→"데이터 범위 밖"으로 바뀌었다
(기간은 골랐는데 데이터가 없는 것이므로 이쪽이 사실이다). 업로드 CSV도 같은 검사를 탄다 —
기간을 반만 덮는 CSV는 "현재 50% · 비교 0%"로 거절된다.
...
### 21. 스키마에서 두 필드를 뺐다 · CSV는 전부 내보낸다 (2026-09-02)
`category`(10종)와 `metric`(traffic/revenue/both)을 이벤트에서 없애고, `divisions`를
**`division`**으로 고쳤다. 같은 날 시드 이벤트 `E156`(IMF 중동 전쟁 영향 성장 하향,
강도 5)도 요청에 따라 제거했다 — 572 → **571건**, 필드 30 → **28종**.

**`metric`은 어디서도 읽히지 않았다.** 저장만 되고 계산·화면 어느 쪽도 쓰지 않는 필드였다.

**`category`는 한 곳에서 진짜로 쓰이고 있었다 — 축 휴리스틱의 첫 분기다.**
`guess_axis()`(파이썬)와 `axisOf()`(JS)가 `category`로 갈라진 뒤에야 경쟁사 이름을 봤다.
그래서 카테고리가 `regulation`/`economy`면 규칙에 아예 도달하지 못했고, **원칙 3에서
애플 기사 13건이 잘못된 축에 놓인 원인이 정확히 이것**이었다. 필드를 빼면서 분기도 없앴다:

```
텍스트에 공급 키워드     → supply
경쟁사 실명 있고 삼성 없음 → share      ← 카테고리와 무관하게 항상 도달
그 외                    → demand
```

**즉 이 제거는 기능 손실이 아니라 원칙 3이 지적한 구멍을 메운 것이다.** "애플 규제 강화"가
이제 share로 나온다. 실측: 571건을 축 없이 재계산해 **파이썬·JS 불일치 0건**(두 사본이
같아야 한다는 원칙 3의 요구를 그대로 만족).

곁가지 하나: `optimize.py`의 `recent_kept_titles()`가 `category != "company" or source`로
시드를 걸러내고 있었다. 대리 조건이었으므로 `date_source != "seed"`로 바꿨다 — 주석이
원래 말하던 것("tend to skip seeds")을 그대로 쓴 것이다.

**CSV 내보내기는 이제 저장된 28개 필드 전부 + `share_pct`(기간 기여 비중) = 29열이다.**
- 열 순서는 `CSV_COLS`에 명시한다(식별 → 분류 → 판정 → 본문 → 출처). **목록에 없는 키가
  원장에 생기면 맨 뒤에 자동으로 붙는다** — 조용히 빠지지 않게.
- `share_pct`는 저장값이 아니라 화면이 계산하는 값이라 내보내기에서만 만든다. 배분 대상이
  아니면 **0이 아니라 빈칸**이다(원칙 17: 0은 "기여가 없었다"로 읽힌다).
- 검증: 529행의 `share_pct` 합이 **정확히 100.00%**.

**같이 고친 버그**: `new Blob(['\ufeff'+csv, {type:...}])` — 옵션 객체가 Blob의 두 번째
인자가 아니라 **파츠 배열 안에** 있었다. `[object Object]`가 문자열화되어 **모든 내보내기
파일 끝에 붙었고**(마지막 행의 마지막 셀이 깨짐) MIME 타입은 빈 문자열이었다.
...
외부 요인       배분 요약(관측 변화 | 역할별 발산 막대) + 상시 경고문  ← 화면의 중심
                [정렬(최신·기여도·영향강도)·방향·축 세그먼트]
                split( 목록(행마다 기여 %와 발산 막대) | 선택한 요인 상세 )
검증            검정 6개를 각자의 기준선과 나란히 (맨 아래)
```

**2026-09-02: `기간 기여 배분`을 독립 섹션에서 `외부 요인` 안으로 합쳤다.** 배분 섹션이
자기 top 12를 따로 그리고 있어서 **같은 이벤트가 한 화면에 두 번** 나왔고, 순위를 보려면
목록이 아닌 다른 곳을 읽어야 했다. 지금은 **합계와 경고문만** 목록 위에 남고, 순위는
정렬 세그먼트의 `기여도순`이다. 행마다 있던 기여 % 뱃지가 발산 막대까지 갖는다.
- 지운 것: `allocPanel`·`allocRows()`·`ALLOC_TOP`·`EVLAST`(top 12 전용이었다)·`.arow` CSS.
- `기여도순`은 **몫의 크기**(`|share|`)로 줄 세운다. 기간의 움직임과 반대로 민 요인도
  같은 크기면 같은 순위이고, 부호는 이미 행에 적혀 있다. 배분 대상이 아닌 건(방향 미상,
  기간 비교 없음)은 0이 아니라 **맨 뒤**로 보낸다 — 0으로 두면 "기여가 없었다"로 읽힌다.
- 막대 길이는 `ALLOC.maxAbs`(기간 전체 최대 몫)에 맞춘다. 보이는 행 중 최대에 맞추면
  **같은 이벤트가 필터에 따라 다른 길이**로 그려지는데, 필터는 배분을 건드리지 않는다는
  게 아래 규칙이다.
- 검증: 기여도순 상위 12건이 **삭제 전 배분 목록의 top 12와 제목·퍼센트까지 12/12 동일**.
  방향·축 필터를 걸어도 합계 줄은 그대로다(423건 · −44.9%).

**같은 날: 목록에서 배분 대상이 아닌 건을 뺐다.** 방향이 없는 요인은 몫이 없어서 행에
퍼센트도 막대도 없었고, **"배분됐는데 기여가 0인 건"처럼 보였다**. 지금은 `ALLOC`이 있으면
`share`가 있는 건만 목록에 낸다(기본 기간 499 → 423건, 방향 미상 76건 제외).
- **사라지는 게 아니라 자리를 옮긴 것이다.** 빠진 76건은 합계 줄의 `방향 미상`이 그대로
  세고, 경고문이 "방향이 없어 배분에서 빠진 건은 목록에 나오지 않습니다 — 위 방향 미상
  76건이 그것입니다"라고 말한다. **CSV 내보내기는 `rows()`를 쓰므로 목록 필터·배분
  제외와 무관하게 기간·지역 전건이 나간다.**
- **KPI `수집 이벤트`가 전체 누적 원장을 맡는다.** 큰 숫자는 항상 `EV.length`(540)이고,
  좁혀지는 과정은 설명줄에 적는다(`기간·지역 499건 · 배분 423건`). 목록이 부분집합만
  보여주게 됐으므로 **원장 전체 크기를 말하는 곳이 화면에 하나는 있어야 한다.**
- `evcount`의 분모도 `ALLOC.n`으로 바꿨다 — `423 / 540`이라고 적으면 목록에 애초에 없던
  117건을 찾아보게 만든다.
- **배분이 아예 없는 기간(기간 미선택·데이터 범위 밖)에는 제외하지 않는다.** 뺄 기준이
  없는데 빼면 목록이 통째로 빈다.
- 고친 버그 1건: `ALLOC` 대입이 **그것을 읽는 KPI 행보다 아래**에 있었다. 첫 렌더에는
  배분 건수가 안 나오고, 이후 렌더에는 **직전 기간의 값**이 찍혔다. 기간 단위 값들과 함께
  render() 맨 위로 올렸다.
...
계산만으로 나옴)과 `기간 기여 비중`(요인별, 라벨이 비율을 정함 — 원칙 17). 둘은 출처가
완전히 다르므로 같은 말을 쓰면 안 된다. (후자는 독립 섹션이던 시절 `기간 기여 배분`이라
불렸다. 섹션이 사라졌으니 용어집·검증 문구가 쓰는 이름은 `기간 기여 비중` 하나다.)

- **목록 필터는 목록에만 건다.** `FDIR`/`FAXIS`/`FSORT`는 `lr`(목록용)에만 적용하고
  3축 분해와 기여 배분은 언제나 기간 전체 `r`을 쓴다 — 둘 다 항등식이라 부분집합으로
