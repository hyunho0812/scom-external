#!/usr/bin/env python3
"""
Layer 3 — first-party source monitor (FREE, no API calls).
...
# --- MyMemory 무료 번역 (키 불필요, 익명 하루 ~5,000단어) ---
# 1차 출처 제목/요약을 한국어로 번역. 실패 시 원문 반환(원본은 raw 필드에 별도 보관).
_tr_cache = {}
def translate_ko(text):
    ...
    # 한국어가 이미 포함되어 있으면 그대로
    if any('\uac00' <= c <= '\ud7a3' for c in text):
        _tr_cache[text] = text
        return text
    snippet = text[:480]  # MyMemory 한 요청 길이 제한 고려
    try:
...
COUNTRIES_ALL = ["US","GB","DE","FR","ES","PT","BR","MX_C","AU","IN","TR","KR"]
